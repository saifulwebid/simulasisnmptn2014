...
@section('inner-content')
	...
	<table class="table" data-graph-container="#chart" data-graph-type="column" data-graph-yaxis-1-stacklabels-enabled="1" data-graph-xaxis-rotation="{{ count($rekap) < 13 ? '0' : '-60' }}" data-graph-xaxis-align="{{ count($rekap) < 13 ? 'center' : 'right' }}" data-graph-yaxis-1-title-text="Jumlah Peminat (orang)" data-graph-subtitle-text="Dikelompokkan berdasarkan peminatan pilihan 1, 2, dan 3">
		<caption style="display: none">Jumlah Peminat Program Studi di {{ $ptn->nama }}</caption>
		<thead>
			<tr>
				<th>Nama Program Studi</th>
				<th data-graph-skip="1">Bidang</th>
		...
		<tbody>
			...
			<tr>
				...
				<td style="vertical-align: middle">{{ $data->bidang }}</td>
				...
				<td style="vertical-align: middle"><a href="{{ URL::route('filter.prodi', array('ptn' => $data->ptn_id, 'prodi' => $data->id)) }}" class="btn btn-primary btn-sm">Lihat Peminat Program Studi Ini</a></td>
			</tr>
			@endforeach
		</tbody>
	</table>
